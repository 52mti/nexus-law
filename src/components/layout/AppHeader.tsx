...
export const AppHeader: React.FC = () => {
  ...
  const mockNotifications = Array(5).fill({
    ...
    detailPara1: '我们将在6月中旬上线图片3.0模型效果优化实验。此次更新不影响您的使用流程，但模型效果会产生变化。如果您正在推进的项目对风格一致性有要求，建议提前完成关键内容产出。',
    detailPara2: '这次更新是我们持续提升模型能力的重要一步，也非常欢迎大家在使用中多多反馈，我们会认真听取并不断优化！',
  ...
  return (
    <>
      <div className='flex items-center justify-end h-full px-6 bg-white border-b border-gray-100 gap-6'>
        ...
        <Popover content={userProfileContent} trigger='hover' placement='bottomRight' arrow={false} color='transparent' styles={{ container: { padding: 0 } }}>
      ...
      {/* ==================== 消息中心抽屉 (Drawer) ==================== */}
      <Drawer
        title={<div className="flex items-center gap-2 text-base"><BellOutlined /> 消息中心</div>}
        closable={false}
        placement="right"
        size={380}
      ...
      >
        <div className="flex flex-col gap-3">
          {mockNotifications.map((notice) => (
            <div key={notice.id} className="bg-white rounded-lg p-4 border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
              <div className="flex items-center gap-2 mb-2">
                {!notice.isRead && <div className="w-1.5 h-1.5 rounded-full bg-red-500 flex-shrink-0"></div>}
                <span className="font-medium text-gray-800 text-[15px] truncate">{notice.title}</span>
              </div>
      ...
      {/* ==================== 🚀 详情系统消息弹窗 (Modal) ==================== */}
      <Modal
        title={<span className="font-bold text-gray-800 text-base">系统消息</span>}
      ...
      <Modal
        ...
        width={720}
        centered
        styles={{ container: { padding: 0, overflow: 'hidden', borderRadius: '12px' } }}
      >
        {/* 深色头部 */}
        ...
        <div className="bg-white p-6 flex gap-8 rounded-b-xl">
          ...
          <div className="w-55 pl-8 border-l border-gray-100 flex flex-col items-center justify-center">
            <h3 className="text-base font-medium text-gray-800 w-full mb-4">扫码支付</h3>
            
            {/* 直接调用提取好的 QRCodeScanner */}
            <QRCodeScanner />
            
            <div className="text-[12px] text-gray-400 text-center mt-3">
              支付即代表您已同意 <a className="text-[#666cff] hover:text-[#585ee6] hover:underline transition-colors">《付费服务协议》</a>
            </div>
          </div>
        </div>
      </Modal>
