    ...
    const fetchPlans = async () => {
      ...
      try {
        ...
        if (res.successful && res.data?.records) {
          ...
          const formattedPlans: UIPlanType[] = res.data.records.map(
            (record) => {
              ...
              return {
                ...
                price: isFree
                  ? t("5orx1DyQY2gLVgf5OsX0b")
                  : Number(record.price).toString(),
                priceUnit: isFree ? "" : t("SN0eJqECHeRaU7DfLdk1l"),
                subtitle: record.hint || "",
                buttonText: isFree
                  ? t("TW3Qxm3H9mjNuyd7A1faX")
                  : t("Sy1xxFPxQB9iXLe3kUKFo"),
                isPrimary: isPrimary,
                buttonDisabled: isFree, // 免费体验无需购买
                pointsTitle: `每月获得 ${record.gainPointsPerMonth || 0} 积分`,
                // 如果有些特定的描述（如黄金会员的图生图数量），前端可以自己写死兜底，或者让后端加字段
                pointsDesc:
                  record.code === "BJHY" ? t("W9l_-1p_h8Sx4CdVjp3AG") : "",
                // 共同的权益说明，后端暂无此字段，前端先统一配置
                features: [
                  t("K_CPY64x49zsOjtNMh25G"),
                  `条文检索：每日${record.legalProvisionsCount === -1 ? t("fP9WDYpLGVPTJ8XjfPZxz") : `限 ${record.legalProvisionsCount} 次`}基础检索`,
                  `案例检索匹配：每日${record.caseCount === -1 ? t("fP9WDYpLGVPTJ8XjfPZxz") : `限 ${record.caseCount} 次`}检索`,
                  `法律文书生成：每日${record.legalDocumentTranslationCount === -1 ? t("Vt_O0DK8M4pHvxNTaer0C") : `限 ${record.legalDocumentTranslationCount} 次`}算力限制`,
          ...
          formattedPlans.sort(
            (a, b) =>
              Number(a.originalData.price) - Number(b.originalData.price),
          );
  ...
  return (
    <div className="h-full overflow-y-auto bg-[#f9fafb] p-8 pb-16 relative flex flex-col items-center">
      ...
      <div className="text-center mt-6 mb-12 animate-fade-in">
        ...
        <p className="text-[15px] text-gray-500 tracking-widest">
          {t("X-6ME5WqeWy9lsGJ9-fAR")}{" "}
        </p>
      </div>
            ...
            >
              ...
              >
                ...
                <div className="flex items-baseline gap-1 mb-2">
                  ...
                  <span
                    className={`text-sm ${isPrimary ? "text-indigo-100" : "text-gray-500"}`}
                  >
                    {plan.priceUnit}
                  </span>
                </div>
